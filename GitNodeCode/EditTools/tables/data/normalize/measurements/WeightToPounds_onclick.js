    ...
    { var TDi=-1, TRi=-1;
       ...
       { var TRs = T.getElementsByTagName("TR");
         ...
         {  
            ...
            var M = orig.match( /([0-9\.\-]+)[ ]*kg/i ); 
            if(M && M.length==2) { rep=parseFloat(M[1]) * LBPerKG; done=1; }
            if (!done){
               var M = orig.match( /([0-9\.]+)[ ]*lb/i ); 
               if(M && M.length==2) { rep=parseFloat(M[1]) ; done=1; }
            } 
            if (!done){
                   var M = orig.match( /([0-9\.]+)/ );    /* default numeric only (assume lbs) */
                 if(M && M.length==2) { rep=parseFloat(M[1]) ; done=1; }
            } 
